app.controller('signupController',['$scope', 'signupService', function($scope, signupService) {
    ...
    $scope.uploadFile = function()
     {
        ...
        blob = $("#upload_imagefile_1")[0].files[0];
        formData.append('media', blob, '2.jpg');
       blob = $("#upload_imagefile_2")[0].files[0];
       formData.append('media', blob, '3.jpg');
       blob = $("#upload_imagefile_3")[0].files[0];
       formData.append('media', blob, '4.jpg');
       blob = $("#upload_imagefile_4")[0].files[0];
       formData.append('media', blob, '5.jpg');
       blob = $("#upload_imagefile_5")[0].files[0];
       formData.append('media', blob, '6.jpg');
       blob = $("#upload_imagefile_6")[0].files[0];
       formData.append('media', blob, '7.jpg');
       blob = $("#upload_file_rider")[0].files[0];
       formData.append('media', blob, '8.jpg');
       blob = $("#upload_file_setlist")[0].files[0];
       formData.append('media', blob, '9.jpg');
       blob = $("#upload_file_prviousclients")[0].files[0];
       formData.append('media', blob, '10.jpg');
       blob = $("#upload_file_testimonials")[0].files[0];
       formData.append('media', blob, '11.jpg');
        ...
        $.ajax("/api/signup/upload", {
            ...
            success: function (files) {
              for (var count = 0; count < files.length; count++) {
               ...
               if(count==3){$scope.Images3=file.filename;}
               if(count==4){$scope.Images4=file.filename;}
               if(count==5){$scope.Images5=file.filename;}
               if(count==6){$scope.Images6=file.filename;}
               if(count==7){$scope.Rider=file.filename;}
               if(count==8){$scope.SetList=file.filename;}
               if(count==9){$scope.Previousclients=file.filename;}
               if(count==10){$scope.Testimonials=file.filename;}
               return 'ok';
              }
            },
            error: function (err) {
  ...
  $scope.ClickSubmit = function() {
    ...
    // if($scope.quickfile.filenameponse==undefined){return;}
    ...
    // if($scope.file.filenameidency_contracts==undefined){return;}  
    ...
    var contentObj = {
     ...
     // quickfile.filenameponse:$scope.quickfile.filenameponse,
   ...
   //   file.filenameidency_contracts:$scope.file.filenameidency_contracts,
  ...
  $scope.$on('$viewContentLoaded', function(){
    ...
    $(document).ready(function () {
      ...
      $('#owl-slider-rtl').owlCarousel( {
          ...
          autoplayHoverPause: true, nav: true, navText: [ "<i class='ti-angle-left'></i>", "<i class='ti-angle-right'></i>"], items: 1, file.filenameponsive: {
              0: {
                  items: 1
              }
              , 479: {
      ...
      $('#featured-owl-rtl').owlCarousel( {
          rtl: true, loop: true, dots: false, nav: false, items: 4, file.filenameponsive: {
              0: {
                  items: 1
              }
              , 479: {
      ...
      $('.post-slider-rtl').owlCarousel( {
          ...
          dots: false, nav: true, navText: [ "<i class='ti-angle-left'></i>", "<i class='ti-angle-right'></i>"], items: 1, file.filenameponsive: {
              0: {
                  items: 1
              }
              , 479: {
      ...
      //progfile.filenameber
      var el=document.getElementsByClassName('progfile.filenamesber'), l=el.length;
      for (var i=0;
      i < l;
      i++) {
          var options= {
